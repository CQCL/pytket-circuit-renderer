...
export default {
  ...
  computed: {
    ...
    hasNestedContent() {
      return (
        (["Condition", "Control", "QControlBox"].includes(this.opType) &&
          "box" in this.op &&
          "op" in this.op.box &&
          this.contentOps.includes(this.op.box.op.type)) ||
        (this.opType === "Condition" &&
          this.contentOps.includes(this.op.conditional.op.type))
      );
    },
    hasContent() {
      return (
        (this.contentOps.includes(this.opType) &&
          (!this.renderOptions.recursive ||
            !["CircBox"].includes(this.opType))) ||
        this.hasLongParams
      );
    },
    hasLongParams() {
    ...
    params() {
      ...
      return params.concat(
        ...
        this.opType === "Condition" && this.op.conditional.op.params
          ? this.op.conditional.op.params
          : []
      );
    },
  },
  methods: {
    ...
    formatBoolMatrix(matrix) {
      ...
      for (const x of Array(2 ** nQubits).keys()) {
        basis.push(
          (new Array(1 + nQubits).join("0") + x.toString(2)).slice(-nQubits)
        );
      }
      let formattedMatrix = [];
    ...
    <div style="display: none">
      <teleport-from :to="visible ? teleportId : ''" :parent="teleportParent">
        <info-modal v-model="visible" class="tool-tip-content">
          <template #title> [[# opType #]] </template>
          <template #content>
            ...
            <div v-if="displayOp.type === 'PhasePolyBox'">
              <div class="row">
                Encapsulating [[# displayOp.box.n_qubits #]] gates
              </div>

              <h4>Qubit Mapping</h4>
              <chart-list
                :chart="
                  formatMapping(displayOp.box.qubit_indices, 'register', false)
                "
              ></chart-list>
              ...
              <chart-list
                :chart="
                  formatMapping(displayOp.box.phase_polynomial, 'bool', false)
                "
              ></chart-list>

              <h4>Linear Transformation</h4>
              <chart-matrix
                :matrix="formatBoolMatrix(displayOp.box.linear_transformation)"
                :display-title="false"
                entry-type="bool"
              ></chart-matrix>
            </div>
            ...
            <table
              v-if="
                ['Custom', 'Composite', 'CompositeGate'].includes(
                  displayOp.type
                )
              "
              class="mathjax-content hover-highlight"
            >
              <tr>
                <th>Parameter</th>
                <th>Value</th>
              ...
              <tr v-for="(param, i) in displayOp.box.params" :key="i">
                <td>
                  <mathjax-content
                    :formula="'\`' + displayOp.box.gate.args[i] + '\`'"
                  ></mathjax-content>
                </td>
                <td>
                  <mathjax-content
