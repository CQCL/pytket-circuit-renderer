...
export default {
  ...
  methods: {
    isControlWire(arg) {
      return (
        this.controlledCommand.args.findIndex((reg) =>
    ...
    <div v-for="arg in renderIndexedArgs" :key="arg.name[0] + '-' + arg.order">
      <div v-if="arg.pos === -1" class="gate_container">
        ...
        <div
          ...
          :class="{ classical: arg.flags.classical }"
        ></div>
      ...
      <div v-else>
        <div v-if="isControlWire(arg.name)">
          <div class="gate_container">
            ...
            ></wire>
            <div
              v-if="(opType === 'Condition') & !arg.flags.condensed"
              class="control_index"
            >
              [[# arg.pos #]]
            </div>
            <div
              class="gate gate_control"
              :class="[arg.flags.classical ? 'classical' : 'z']"
            ></div>
            <div v-if="arg.order > 0" class="link-top"></div>
          </div>
        </div>
