...
export default {
  ...
  computed: {
    ...
    specialGateContentClasses() {
      if (this.opType === "Reset") {
        return [
          "gate",
          "gate_box",
          { "zx-spider x": this.renderOptions.zxStyle },
        ];
      }
      return ["gate_name"];
    },
    ...
    name() {
      ...
      if (
        ["Custom", "Composite", "CompositeGate"].includes(this.opType) &&
        "box" in op &&
        "gate" in op.box
      ) {
        name =
          "name" in op.box.gate ? op.box.gate.name : "Parametrised Circuit";
      }
      if (
        ["ExplicitPredicate", "ExplicitModifier"].includes(this.opType) &&
        "classical" in op
      ) {
      ...
      if (this.opType === "MultiBit" && "classical" in op) {
        name = `Multi-${
          "name" in op.classical ? op.classical.name : "Classical"
        }`;
      }
  ...
  methods: {
    asSpider(opType) {
      return ["X", "U1", "V", "Vdg", "Y", "Z", "S", "Sdg", "T", "Tdg"].includes(
        opType
      );
    },
...
<template>
  <div :data-gate-component="opType">
    ...
    <div
      v-if="opType === 'Barrier'"
      :class="{ 'gate_container nested': arg.flags.single }"
    ...
    >
      ...
      <div class="gate_container" :class="[gateColor]">
        <div
          ...
          class="barrier"
          :class="[
            arg.flags.first ? 'link-bottom edge-link' : 'link-top',
            { 'edge-link': arg.flags.last },
          ]"
        ></div>
      </div>
    </div>
    ...
    >
      <wire
        v-if="arg.pos !== -1"
        class="wire_in"
        :class="{ flex_wire: arg.flags.single }"
        :classical="arg.flags.classical"
        :condensed="arg.flags.condensed"
      ></wire>
      ...
      <div
        ...
        :class="[gateColor, { generic: !arg.flags.single }]"
      >
        <div class="gate" :class="specialGateClasses">
          ...
          <span
            :class="specialGateContentClasses"
            :style="[opType === 'Reset' ? { margin: 0 } : {}]"
          >
            [[# arg.flags.last ? name : '' #]]
          </span>
        </div>
