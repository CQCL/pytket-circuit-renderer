...
export default {
  ...
  computed: {
    ...
    classicalTable() {
      const classical =
        this.op && "classical" in this.op ? this.op.classical : this.op.box;
    ...
    hasClassicalTable() {
      return Object.values(this.classicalTable).reduce(
        (prev, next) => prev || typeof next !== "undefined",
        false
      );
    },
  },
    ...
    formatClassicalExp(expression) {
      ...
      for (let arg of expression.args) {
        if (typeof arg == "number") formattedArgs.push(arg); // constant
          ...
          formattedArgs.push(`${arg[0]}[${arg[1][0]}]`); // register
      ...
      if (n === 0) return operation; // -> op
        ...
        return `(${formattedArgs[0]} ${operation} ${formattedArgs[1]})`; // -> (exp op exp)
